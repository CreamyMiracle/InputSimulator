...
namespace InputCollector.Hooks
{
    public class KeyboardHook
    {
        public void SetHook()
        {
            Task.Run(() => 
            {
                while (true)
                {
                    for (int i = 0; i < 255; i++)
                    {
                        int key = Win32API.GetAsyncKeyState(i);
                        if (key == 1 || key == 32769)
                        {
                            Console.WriteLine(verifyKey(i));
                            KeyboardEvent(this, new KeyboardEvent(i));
                            break;
                        }
                    }
                }
        ...
        public delegate void MouseClickHandler(object sender, KeyboardEvent e);
        public event MouseClickHandler KeyboardEvent;
    }
}
